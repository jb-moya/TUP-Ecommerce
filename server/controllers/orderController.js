    ...
    if (cart) {
        ...
        if (order) {
            ...
            for (const item of cart.orderItems) {
                ...
                if (product) {
                    const existingSellerIndex = order.orders.findIndex(
                        (order) =>
                            order.seller &&
            ...
            res.status(400).json({
                message: "An order does not exist for this user.",
            });
        }
        ...
        res.status(400).json({
            message: "A cart does not exist for this user.",
        });
    }
});
...
const deleteOrder = asyncWrapper(async (req, res, next) => {
    ...
    if (!order) {
        return next(createCustomError(`No cart with id : ${OrderID}`, 404));
    }
